...
| `supabase/migrations/016_task_group_title.sql` | ☑ | ? | adds `tasks.task_group_title text`. Kid-facing bundle name shown on the Tavle bundle row. Vision extractor suggests one (`groupTitle` field) and parent can edit before commit. Denormalised across siblings — same value on every row in a batch — so a single SELECT carries it. Nullable; pre-016 bundles fall back to "N opgaver fra ét lektiebillede". Applied on dev 2026-04-28. |
| `supabase/migrations/017_task_expected_answers.sql` | ☑ | ? | adds `tasks.task_expected_answers jsonb`. Per-step expected answers from the vision extractor are now persisted (previously preview-only). Used by ParentTaskPreview to render "Forventet svar" next to each step + by the AddToBoardForm review queue (editable, with auto-sync into tutor-kontekst). Index-aligned with `task_steps`; empty strings preserved for unreadable items. Applied on dev 2026-04-28. |
